...
const version = (process.env.MIMO_WORK_APP_VERSION || pkg.version || '').trim()
if (!version) {
  console.error('[zip-mac-app] Could not resolve package version.')
  process.exit(1)
}

const distDir = resolve(process.env.MIMO_WORK_DIST_DIR || join(root, 'dist'))
const appOutDir = join(distDir, arch === 'arm64' ? 'mac-arm64' : 'mac')
const appName = 'MIMO Work.app'
const appPath = join(appOutDir, appName)
const zipPath = join(distDir, `MIMO-Work-${version}-mac-${arch}.zip`)
