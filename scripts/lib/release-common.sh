...
release_export_update_channel() {
  ...
  export MIMO_WORK_UPDATE_CHANNEL="${RELEASE_CHANNEL}"
...
release_load_local_env() {
  local env_file="${MIMO_WORK_RELEASE_ENV:-${KUN_RELEASE_ENV:-}}"
...
release_compute_version() {
  ...
  if [[ "${RELEASE_BUMP:-}" == "none" && -n "${RELEASE_TAG:-}" ]]; then
    ...
    RELEASE_NAME="MIMO Work ${RELEASE_VERSION}"
    LATEST_TAG=""
    return
  fi
...
release_export_app_version() {
  ...
  export MIMO_WORK_APP_VERSION="${RELEASE_VERSION}"
  cyan "  App:     ${MIMO_WORK_APP_VERSION}"
}
...
release_clean_dist_artifacts() {
  ...
  rm -f "${ROOT}"/dist/MIMO-Work-* "${ROOT}"/dist/MIMO\ GUI-* "${ROOT}"/dist/latest*.yml "${ROOT}"/dist/*.blockmap
}
