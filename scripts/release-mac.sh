...
copy_mac_arch_artifacts() {
  ...
  shopt -s nullglob
  files=("${output_dir}"/MIMO-Work-*-mac-"${arch}".*)
  shopt -u nullglob
...
# artifactName: MIMO-Work-${version}-mac-${arch}.dmg|zip
collect "macOS arm64 dmg" "dist/MIMO-Work-*-mac-arm64.dmg"
collect "macOS x64 dmg" "dist/MIMO-Work-*-mac-x64.dmg"
collect "macOS arm64 zip" "dist/MIMO-Work-*-mac-arm64.zip"
collect "macOS x64 zip" "dist/MIMO-Work-*-mac-x64.zip"
collect_optional "macOS blockmap" "dist/MIMO-Work-*-mac-*.zip.blockmap"
...
cyan "  https://github.com/MIMO-Work/MIMO-Work/releases/tag/${TAG_NAME}"
