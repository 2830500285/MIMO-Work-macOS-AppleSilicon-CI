...
function effectiveMimoCredentials(settings: AppSettingsV1) {
  ...
  const envApiKey = process.env.MIMO_WORK_MIMO_API_KEY?.trim() || ''
  const directMimo = envApiKey
    ? normalizeMimoCredentialSettings({ ...runtime.mimo, apiKey: envApiKey })
    : runtime.mimo
  if (!providerKey) return directMimo
