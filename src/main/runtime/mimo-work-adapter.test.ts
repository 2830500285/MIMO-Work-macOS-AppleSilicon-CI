...
describe('mimo work adapter contract mapping', () => {
  ...
  it('starts MiMo-Code with a minimal redacted environment', async () => {
    const { mimoWorkAdapterTestInternals } = await import('./mimo-work-adapter')
    const isolatedHome = '/Users/test/.mimo-work/data/mimocode/home'
  ...
  it('selects Windows MiMo-Code .exe candidates for the x64 bundle', async () => {
    const { mimoWorkAdapterTestInternals } = await import('./mimo-work-adapter')

    expect(
      mimoWorkAdapterTestInternals.mimoBinaryCandidates('/opt/MIMO-Work-Core', 'win32', 'x64')
        .map((candidate) => candidate.replace(/\\/g, '/'))
    ).toEqual([
      '/opt/MIMO-Work-Core/packages/opencode/dist/mimocode-windows-x64/bin/mimo.exe',
      '/opt/MIMO-Work-Core/packages/opencode/dist/mimocode-windows-x64-baseline/bin/mimo.exe',
      '/opt/MIMO-Work-Core/dist/mimocode-windows-x64/bin/mimo.exe',
      '/opt/MIMO-Work-Core/dist/mimocode-windows-x64-baseline/bin/mimo.exe'
    ])
    expect(mimoWorkAdapterTestInternals.mimoCorePlatform('win32')).toBe('windows')
    expect(mimoWorkAdapterTestInternals.mimoCoreArch('ia32')).toBe('x64')
    expect(mimoWorkAdapterTestInternals.mimoBinaryName('win32')).toBe('mimo.exe')
  })

  it('resolves the bundled Windows executable and uses the core directory as cwd', async () => {
    const { mimoWorkAdapterTestInternals } = await import('./mimo-work-adapter')
    const root = await mkdtemp(join(tmpdir(), 'mimo-work-core-'))
    const binary = join(root, 'packages', 'opencode', 'dist', 'mimocode-windows-x64', 'bin', 'mimo.exe')
    const previousCoreDir = process.env.MIMO_WORK_CORE_DIR
    const previousBinPath = process.env.MIMOCODE_BIN_PATH

    await mkdir(join(binary, '..'), { recursive: true })
    await writeFile(binary, '', 'utf8')
    process.env.MIMO_WORK_CORE_DIR = root
    delete process.env.MIMOCODE_BIN_PATH

    try {
      const launch = mimoWorkAdapterTestInternals.resolveLaunch(settings(), 4567, 'win32', 'x64')
      expect(launch.command).toBe(binary)
      expect(launch.cwd).toBe(root)
      expect(launch.args).toEqual(['serve', '--hostname', '127.0.0.1', '--port', '4567'])

      const spawnOptions = mimoWorkAdapterTestInternals.mimoRuntimeSpawnOptions(
        launch,
        'C:\\Users\\test\\.mimo-work\\data\\mimocode',
        { MIMOCODE_HOME: 'C:\\Users\\test\\.mimo-work\\data\\mimocode' }
      )
      ...
      if (previousCoreDir === undefined) {
        delete process.env.MIMO_WORK_CORE_DIR
      } else {
        process.env.MIMO_WORK_CORE_DIR = previousCoreDir
      }
      if (previousBinPath === undefined) {
        delete process.env.MIMOCODE_BIN_PATH
      } else {
        process.env.MIMOCODE_BIN_PATH = previousBinPath
      }
      await rm(root, { recursive: true, force: true })
    }
  })

  it('preserves the Windows process path needed to spawn MiMo-Code without leaking secrets', async () => {
    const { mimoWorkAdapterTestInternals } = await import('./mimo-work-adapter')
    const env = mimoWorkAdapterTestInternals.mimoCoreChildEnv({
      ...
      MIMO_API_KEY: 'sk-should-not-leak'
    }, {
      MIMOCODE_HOME: 'C:\\Users\\test\\.mimo-work\\data\\mimocode',
      MIMOCODE_CLIENT: 'desktop'
    }, 'C:\\Users\\test\\.mimo-work\\data\\mimocode\\home', 'win32')

    expect(env).toMatchObject({
      PATH: 'C:\\Windows\\System32;C:\\Windows',
      HOME: 'C:\\Users\\test\\.mimo-work\\data\\mimocode\\home',
      USERPROFILE: 'C:\\Users\\test',
      SystemRoot: 'C:\\Windows',
      ComSpec: 'C:\\Windows\\System32\\cmd.exe',
      PATHEXT: '.COM;.EXE;.BAT;.CMD',
