...
function readWeixinPackageInfo(): WeixinPackageInfo {
  ...
  if (!packageJson) {
    throw new Error(
      'Built-in WeChat login component is missing. Reinstall MIMO Work or rebuild with @tencent-weixin/openclaw-weixin bundled.'
    )
  }
...
function buildBaseInfo(): JsonRecord {
  ...
  return {
    ...
    bot_agent: `MIMO-Work/${app.getVersion() || '0.0.0'}`
  }
}
    ...
    switch (recordString(status, 'status')) {
      ...
      case 'binded_redirect':
        ...
        return {
          ...
          sessionKey,
          message: '已连接过此 MIMO Work，无需重复连接。'
        }
...
async function postToMimoWorkWebhook(message: WeixinMessage, accountId: string): Promise<JsonRecord> {
  ...
  if (settings.webhookSecret) {
    ...
    headers['x-mimo-work-secret'] = settings.webhookSecret
  }
  ...
  if (!res.ok || data.ok === false) {
    throw new Error(recordString(data, 'message') || `MIMO Work webhook HTTP ${res.status}`)
  }
  return data
}
