...
const DEFAULT_WORKSPACE_ROOT = join(homedir(), 'MIMO Work', 'default_workspace')
  ...
  async load(): Promise<AppSettingsV1> {
    ...
    try {
      ...
      if (error instanceof SyntaxError) {
        ...
        if (backupPath) {
          console.warn(
            `[mimo-work] Invalid settings JSON was replaced with defaults. Backup: ${backupPath}`
          )
        } else {
          console.warn(
            `[mimo-work] Invalid settings JSON was replaced with defaults. Backup could not be written for ${sourcePath}.`
          )
        }
        return defaults
      }
