...
describe('plan-request', () => {
  ...
  it('skips internal GUI plan prompts', () => {
    const blocks: ChatBlock[] = [
      { kind: 'user', id: 'u1', text: 'make auth nicer' },
      {
        kind: 'user',
        id: 'u2',
        text: 'MIMO Work is asking you to draft a GUI-owned implementation plan.'
      },
      {
        kind: 'user',
        id: 'u3',
        text: 'Please read and execute the GUI plan file at `.mimo-work/plan/auth.md`'
      },
      {
        kind: 'user',
        id: 'u4',
  ...
  it('recognizes internal plan prompts', () => {
    expect(isInternalGuiPlanPrompt('MIMO Work is asking you to revise an existing GUI-owned implementation plan.')).toBe(true)
    expect(isInternalGuiPlanPrompt('Please read and execute the GUI plan file at `.mimo-work/plan/a.md`')).toBe(true)
