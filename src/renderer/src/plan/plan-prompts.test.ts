...
describe('plan-prompts', () => {
  it('builds draft prompts that route through the native create_plan tool', () => {
    const prompt = buildDraftPlanPrompt({
      ...
      planRelativePath: '.mimo-work/plan/add-auth.md'
    })
  ...
  it('formats internal plan prompts for chat display', () => {
    ...
    expect(formatGuiPlanPromptForDisplay(buildPlanBuildPrompt('.mimo-work/plan/add-auth.md'))).toBe(
      'Build plan: .mimo-work/plan/add-auth.md'
    )
    ...
    expect(getGuiPlanPromptKind('Build plan: .mimo-work/plan/add-auth.md')).toBe('build')
    expect(isGuiPlanDraftOrRefinePrompt('Revise plan: Make it smaller')).toBe(true)
    expect(isGuiPlanDraftOrRefinePrompt('Build plan: .mimo-work/plan/add-auth.md')).toBe(false)
  })
  ...
  it('builds execution prompts that point at the plan file', () => {
    expect(buildPlanBuildPrompt('.mimo-work/plan/add-auth.md')).toContain(
      'Please read and execute the GUI plan file at `.mimo-work/plan/add-auth.md`'
    )
  })
