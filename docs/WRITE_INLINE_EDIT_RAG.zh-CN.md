...
- 用户手动做一次性短语替换时，会先用确定性规则把同段其他同短语一起替换，例如 `mimo gui -> MIMO Work`。
...
此外，术语大小写和简单重命名不完全依赖模型。编辑器会对一次性短语替换做同段传播：当你把 `mimo gui` 改成 `MIMO Work` 或 `DXGUI`，同一个自然段里其他 `mimo gui` 会同步替换。这个确定性层负责“必须发生”的一致性，recent edits 和 RAG 再负责后续 AI 编辑时理解这种意图。
