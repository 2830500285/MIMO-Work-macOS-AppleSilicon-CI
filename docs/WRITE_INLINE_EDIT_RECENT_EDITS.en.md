...
1. The user manually changes the first `MIMO Work` to `Write mode`.
...
- Will check word boundaries to avoid accidentally replacing local strings in `mymimo gui`.
